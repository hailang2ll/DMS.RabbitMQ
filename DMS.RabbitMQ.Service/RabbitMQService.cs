...
namespace DMS.RabbitMQ.Service
{
    public class RabbitMQService
    {
        ...
        private const string RabbitMqAttribute = "RabbitMQAttribute未定义";
        ...
        static void CreateConnection()
        {
            if (_conn == null)
            {
                ...
                {
                    throw new Exception("加载RabbitMQ.config的配置文件失败，" + ex.Message);
                }
            }
        }
        ...
        public void Publish(string exchange, string queue, string routingKey, string body, bool isProperties = false)
        {
            ...
            try
            {
                ...
                channel.BasicPublish(exchange, routingKey, properties, body.StrToByte());
            }
            catch (Exception ex)
            {
